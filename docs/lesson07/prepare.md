...
In the linked list shown above, the first node is called the **head**. If you know where the head is, then you can traverse the entire linked list by following the pointers. Most linked lists maintain a bidirectional linking between nodes. Bidirectional means that each node will maintain a pointer to both the next node and **previous node**. The **doubly-linked list** shown below has both a head and a **tail**.
...
<dd>A linked list that is bidirectional. The linked list will maintain both a head and a tail. To traverse in either direction, the node will have both a pointer to the next node and the previous node. Access to both the head and tail is O(1).</dd>
<dt>head</dt>
